...
| Vai trò | Địa chỉ |
|---|---|
| Router U-Boot (ipaddr) | `192.168.1.1` (cả 32X6 và NR3053 khi nạp TFTP) |
| TFTP server (serverip) | `192.168.1.254` |
| LAN sau boot (Linux) | `192.168.2.1` (32X6) / `192.168.10.1` (NR3053) |
...
Cài `picocom` (Linux) hoặc tải `PuTTY` (Windows). Thông số kết nối: baud **115200 8N1**, cáp TTL **3.3 V**.
...
- **Thoát picocom:** Nhấn `Ctrl+A` rồi `Ctrl+X`.

**Trên Windows (PuTTY):**
1. Mở PuTTY, mục **Connection type** chọn **Serial**.
2. **Serial line**: Điền cổng COM của bạn (ví dụ `COM3` - xem trong Device Manager).
3. **Speed**: `115200`.
4. Nhấn **Open** để kết nối.
- **Thoát PuTTY:** Tắt cửa sổ PuTTY.

> ⚠️ **Lưu ý chung:** Phải **đóng picocom/PuTTY** trước khi chạy lệnh `mtk_uartboot` vì cổng serial chỉ cho phép một phần mềm sử dụng tại một thời điểm.

- Đọc log boot: nhấn **phím bất kỳ** khi thấy dòng `Hit any key to stop autoboot`.
- Bạn có thể gõ lệnh U-Boot trực tiếp trong cửa sổ terminal (ví dụ `bootmenu`, `printenv`).
...
|---|---|
| `mtk_uart/mtk_uartboot` | Tool chạy trên PC (tải ngoài repo) |
| `build_dir/.../arm-trusted-firmware-mediatek-mt7981-ram-ddr3/.../bl2.bin` | NR3053 & 32X6 — BL2 RAM payload (Lấy từ kết quả build) |
...
1. **Đóng picocom/PuTTY** (cổng serial cần trống).
2. Chạy lệnh tại **thư mục gốc của repo**:
...
4. Mở lại picocom/PuTTY (kết nối lại Serial) — đợi prompt U-Boot.
...
# 32X6:
scp -O immortalwrt-mediatek-filogic-viettel_32x6-bl31-uboot.fip root@192.168.1.1:/tmp/fip.bin
```
...
- Đóng `picocom` / `PuTTY` trước khi chạy `mtk_uartboot` (cổng serial chỉ một process).
- NR3053 và 32X6 dùng chung file RAM payload MT7981 DDR3 (`bl2.bin`) tạo ra trong thư mục `build_dir`.
